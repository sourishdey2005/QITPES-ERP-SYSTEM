...
const WorkflowBuilder: React.FC = () => {
  ...
  const createWorkflow = useMutation({
    mutationFn: async (newWF: any) => {
      ...
      const stepsConfig = Array.from({ length: parseInt(newWF.steps) }, (_, i) => ({
        stage: i + 1,
    ...
    <div className="space-y-8 page-transition text-black">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          ...
          className="flex items-center px-8 py-3.5 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl shadow-blue-500/30 hover:bg-blue-700 transition-all gap-3"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         ...
         <MetricCard label="Governance Health" value="100%" icon={<ShieldCheck size={24} />} trend="Verified" color="text-blue-600" />
      </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-[48px] shadow-2xl w-full max-w-xl overflow-hidden border border-slate-200">
              <div className="p-10 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div>
                  <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Logic Configuration</h3>
                  <p className="text-xs font-bold text-blue-600 uppercase tracking-widest mt-1">Design automated approval chain</p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-12 space-y-8">
                <div className="space-y-2">
                  ...
                  <input required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full p-5 bg-slate-50 border border-slate-200 rounded-[20px] outline-none font-bold text-slate-900 focus:ring-8 focus:ring-blue-500/5 transition-all text-lg" placeholder="e.g. Asset Acquisition Level 4" />
                </div>
                ...
                <button disabled={createWorkflow.isPending} type="submit" className="w-full py-6 bg-blue-600 text-white rounded-[24px] font-black text-sm uppercase tracking-[0.3em] shadow-2xl shadow-blue-500/40 hover:bg-blue-700 transition-all flex items-center justify-center gap-4 group">
                  {createWorkflow.isPending ? <Loader2 className="animate-spin" /> : <><Zap size={20} className="group-hover:scale-125 transition-transform" /> Register Protocol</>}
                </button>
              </form>
            </motion.div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {isLoading ? (
          ...
          <motion.div 
            ...
            className="bg-white p-10 rounded-[48px] border border-slate-200 shadow-sm hover:border-blue-400 hover:shadow-2xl transition-all cursor-pointer group relative overflow-hidden flex flex-col"
          >
            <div className="flex items-center justify-between mb-10 relative z-10">
              <div className="p-5 bg-blue-50 text-blue-600 rounded-[24px] group-hover:bg-blue-600 group-hover:text-white transition-all duration-700">
                <GitMerge size={28} />
              </div>
            ...
            <div className="relative z-10 flex-1">
              <h3 className="font-black text-slate-900 text-2xl mb-2 group-hover:text-blue-600 transition-colors leading-tight tracking-tight">{wf.name}</h3>
            ...
            <div className="flex items-center justify-between pt-8 border-t border-slate-50 mt-8 relative z-10">
              <div className="flex items-center gap-4">
                <div className="flex -space-x-3">
                   {[...Array(Math.min(wf.steps, 3))].map((_, x) => (
                     <div key={x} className="w-8 h-8 rounded-full bg-blue-100 border-4 border-white flex items-center justify-center text-[10px] font-black text-blue-600">
                        {x+1}
                     </div>
                   ))}
                </div>
              ...
              <ArrowRight size={24} className="text-slate-200 group-hover:text-blue-600 group-hover:translate-x-3 transition-all duration-500" />
            </div>
            
            {/* Visual background hint */}
            <div className="absolute -bottom-20 -right-20 w-48 h-48 bg-slate-50 rounded-full group-hover:scale-[2] transition-transform duration-1000 -z-0 opacity-40 group-hover:bg-blue-50" />
          </motion.div>
        ))}
      </div>
    </div>
...
const MetricCard = ({ label, value, icon, trend, color = "text-slate-900" }: any) => (
  <div className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm flex items-center justify-between relative overflow-hidden group hover:border-blue-100 transition-all">
    ...
    <div className="p-5 bg-slate-50 text-slate-200 rounded-3xl group-hover:bg-blue-50 group-hover:text-blue-200 transition-all duration-700 relative z-10">
       {icon}
    </div>
  </div>
);
