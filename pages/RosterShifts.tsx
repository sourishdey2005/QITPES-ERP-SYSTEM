...
import {
  ...
  Monitor, Coffee, Zap, Trash2, ShieldCheck, AlertTriangle, Edit2
} from 'lucide-react';
...
const RosterShifts: React.FC = () => {
  ...
  const assignShift = useMutation({
    mutationFn: async (assign: any) => {
      if (editAssignmentId) {
        const { data, error } = await supabase.from('shift_assignments').update(assign).eq('id', editAssignmentId).select();
        if (error) throw error;
        return data;
      } else {
  ...
  const openAssignModal = (assignment?: any) => {
    if (assignment) {
    ...
    <div className="space-y-8 page-transition text-black">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="bg-red-600 text-white px-6 py-3.5 rounded-[20px] font-black text-xs uppercase tracking-widest shadow-2xl shadow-red-500/30 hover:bg-red-700 transition-all flex items-center gap-3"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
              <h3 className="font-black text-slate-900 uppercase tracking-tighter flex items-center gap-2">
                <Monitor size={20} className="text-red-600" /> Site Deployment: <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="bg-transparent border-none outline-none font-black text-red-600 uppercase" />
              </h3>
            </div>
            <table className="w-full text-left">
              <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-10 py-6 text-right">Status / Actions</th>
                </tr>
              </thead>
                    ...
                    <td className="px-10 py-6 text-right">
                      <div className="flex items-center justify-end gap-3">
                        ...
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                          <button onClick={() => openAssignModal(a)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg">
                            <Edit2 size={14} />
                          </button>
        ...
        <div className="space-y-8">
          <div className="bg-white p-10 rounded-[40px] border border-slate-200 shadow-sm group hover:border-red-200 transition-all">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-8 flex items-center gap-2">
              <RotateCcw size={16} className="text-red-500" /> Active Patterns
            </h3>
            <div className="space-y-6">
              {loadingShifts ? <Loader2 className="animate-spin mx-auto text-red-600" /> :
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-[48px] shadow-2xl w-full max-w-xl overflow-hidden border border-slate-200">
              <div className="p-10 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">{editAssignmentId ? 'Modify Assignment' : 'Staff Assignment'}</h3>
                <button onClick={closeAssignModal} className="text-slate-400 hover:text-slate-600 p-3 hover:bg-white rounded-full transition-all shadow-sm"><X size={24} /></button>
              </div>
              <form onSubmit={(e) => { e.preventDefault(); assignShift.mutate(assignForm); }} className="p-12 space-y-8">
                ...
                <button disabled={assignShift.isPending || !assignForm.employee_id || !assignForm.shift_id} type="submit" className="w-full py-6 bg-red-600 text-white rounded-[24px] font-black text-sm uppercase tracking-[0.3em] shadow-2xl shadow-red-500/40 hover:bg-red-700 transition-all flex items-center justify-center gap-4 disabled:opacity-50 disabled:cursor-not-allowed">
