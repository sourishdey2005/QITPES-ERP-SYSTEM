  ...
  return (
    <div className="space-y-6 page-transition">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={() => openModal()}
            className="flex items-center justify-center px-6 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-black shadow-lg shadow-blue-500/20 transition-all text-xs uppercase tracking-widest"
          >
            ...
            >
              <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mt-1">Enterprise Configuration Console</p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-10 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-500/5 transition-all"
                    />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium outline-none focus:ring-4 focus:ring-blue-500/5 h-24 resize-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      placeholder="0"
                      className="w-full p-4 bg-blue-50/30 border border-blue-100 rounded-2xl text-sm font-black outline-none focus:ring-4 focus:ring-blue-500/5"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      placeholder="0"
                      className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-black outline-none focus:ring-4 focus:ring-blue-500/5"
                    />
                  </div>
                </div>
                ...
                <button
                  ...
                  type="submit"
                  className="w-full py-5 bg-blue-600 text-white rounded-[24px] font-black text-sm uppercase tracking-[0.2em] shadow-2xl shadow-blue-500/30 hover:bg-blue-700 transition-all flex items-center justify-center gap-3"
                >
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative">
        ...
        <div className="absolute -bottom-10 -right-10 w-32 h-32 bg-blue-50 rounded-full blur-3xl opacity-50"></div>
      </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects?.map((project: any, i: number) => (
            <motion.div
              ...
              className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden flex flex-col group hover:border-blue-400 hover:shadow-2xl transition-all relative"
            >
              <div className="p-8 flex-1">
                <div className="flex items-start justify-between mb-6">
                  <div className="flex flex-col gap-1">
                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest inline-block w-fit ${project.status === 'Active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' :
                      project.status === 'Planning' ? 'bg-blue-50 text-blue-600 border border-blue-100' :
                        'bg-slate-50 text-slate-500'
                      }`}>
                      {project.status}
                    </span>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter flex items-center gap-1 mt-1">
                      <Building2 size={10} className="text-blue-500" /> {project.client_name || 'Direct Enterprise'}
                    </p>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => openModal(project)} className="p-2 text-slate-300 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all opacity-0 group-hover:opacity-100"><Edit2 size={14} /></button>
                ...
                <h3 className="font-black text-slate-900 text-xl mb-2 group-hover:text-blue-600 transition-colors tracking-tighter leading-tight uppercase">{project.name}</h3>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="w-full bg-slate-50 border border-slate-100 rounded-full h-3 overflow-hidden p-0.5">
                      <motion.div
                        ...
                        className={`h-full rounded-full ${project.utilization > 90 ? 'bg-red-500' : project.utilization > 50 ? 'bg-blue-500' : 'bg-emerald-500'}`}
                      />
                    </div>
                  </div>
              ...
              <div className="p-6 border-t border-slate-50 bg-slate-50/10 flex items-center justify-between relative overflow-hidden group/audit">
                ...
                <button className="flex items-center text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] relative z-10 hover:text-blue-800 transition-colors">
                  Site Audit <ChevronRight size={14} className="ml-1 group-hover/audit:translate-x-1 transition-transform" />
                </button>
                <div className="absolute inset-x-0 bottom-0 h-0 group-hover/audit:h-full bg-blue-50/50 transition-all duration-500 -z-0"></div>
              </div>
            </motion.div>
          ))}
