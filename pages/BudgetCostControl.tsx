...
const COLORS = ['#EA4643', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899', '#06b6d4', '#14b8a6'];
                ...
                <div className="flex flex-wrap bg-slate-100/50 p-1 rounded-[24px] border border-slate-200">
                    {(['overview', 'budgets', 'approvals', 'costcenters', 'pettycash', 'fixed', 'capex', 'projects'] as const).map(tab => (
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-[18px] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white text-red-600 shadow-sm border border-slate-200' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                                    ...
                                    <div className="h-[300px]">
                                        <ResponsiveContainer>
                                            <BarChart data={analytics.budgetVsActual}>
                                                ...
                                                <Bar dataKey="actual" fill="#EA4643" radius={[4, 4, 0, 0]} />
                                            </BarChart>
                                        </ResponsiveContainer>
                                    </div>
                                </VisualBox>
                    ...
                    {activeTab === 'budgets' && (
                        <div className="bg-white rounded-[40px] border border-slate-200 overflow-hidden shadow-sm">
                            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                                ...
                                <button onClick={() => openModal('addBudget')} className="flex items-center gap-2 px-6 py-2 bg-red-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest"><Plus size={14} /> Add New Budget</button>
                            </div>
                                ...
                                {db?.approvals.map(app => (
                                    <div key={app.id} className="bg-white border border-slate-200 p-8 rounded-[40px] flex items-center justify-between group">
                                        <div>
                                            ...
                                            <p className="text-xl font-black text-red-600">{formatCurrency(app.amount)}</p>
                    ...
                    {activeTab === 'costcenters' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-black">
                            {db?.centers.map(center => (
                                <div key={center.id} className="bg-white p-10 rounded-[48px] border border-slate-200 shadow-xl shadow-slate-200/20 group hover:border-red-500 transition-all flex flex-col justify-between h-72">
                                    <div>
                                        <div className="w-12 h-12 bg-red-50 text-red-600 rounded-2xl flex items-center justify-center mb-6"><Zap size={20} fill="currentColor" /></div>
                                    ...
                                    <div className="flex justify-between items-center border-t pt-6">
                                        <span className="text-[9px] font-black uppercase tracking-widest text-red-600">Active Audit node</span>
                            ...
                            <button onClick={() => openModal('addCenter')} className="bg-slate-50 border-2 border-dashed border-slate-200 p-10 rounded-[48px] flex flex-col items-center justify-center text-slate-400 hover:text-red-600 transition-all h-72 group">
                    ...
                    {activeTab === 'pettycash' && (
                        <div className="bg-white rounded-[40px] border border-slate-200 overflow-hidden">
                            <div className="p-8 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
                                ...
                                <button onClick={() => openModal('addPetty')} className="px-6 py-2 bg-red-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest tracking-tighter">Log Entry</button>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {db?.fixed.map(f => (
                                <div key={f.id} className="bg-white p-8 rounded-[40px] border border-slate-200 group">
                                    ...
                                    <p className="text-2xl font-black text-red-600">{formatCurrency(f.amount)}</p>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase mt-4">Recurring on Day: {f.due_day || 'N/A'}</p>
                                </div>
                            ))}
                            <button onClick={() => openModal('addFixed')} className="bg-slate-50 border-2 border-dashed border-slate-200 p-8 rounded-[40px] flex items-center justify-center text-slate-400 h-48 hover:text-red-600">
                                ...
                                <tbody>
                                    {db?.capex.map(c => (
                                        <tr key={c.id} className="hover:bg-slate-50/50">
                                            ...
                                            <td className="px-10 py-5">
                                                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${c.status === 'Purchased' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                                                    {c.status}
                                                </span>
                                            </td>
                                            <td className="px-10 py-5 text-right">
...
const StatCard = ({ label, value, icon, color }: any) => (
    <div className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm relative overflow-hidden group">
        <div className="relative z-10">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${color === 'blue' ? 'bg-red-50 text-red-600' : color === 'emerald' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
        ...
        <div className="absolute top-0 right-0 w-24 h-24 bg-slate-50 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 group-hover:bg-red-50 transition-all"></div>
    </div>
);
...
const BudgetProgressItem = ({ item, onEdit, onDelete }: any) => {
    ...
    return (
        <div className="bg-slate-50 p-6 rounded-[32px] border border-slate-100 hover:border-red-200 transition-all">
            <div className="flex items-center justify-between mb-4">
                <h4 className="font-black text-slate-900 uppercase text-lg tracking-tight">{item.department}</h4>
                <div className="flex items-center gap-2">
                    <button onClick={onEdit} className="p-2 text-slate-400 hover:text-red-600"><Edit2 size={14} /></button>
            ...
            <div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden shadow-inner">
                <motion.div initial={{ width: 0 }} animate={{ width: `${percent}%` }} className={`h-full ${percent > 90 ? 'bg-red-500' : 'bg-red-600'}`} />
            </div>
        </div>
    );
};
