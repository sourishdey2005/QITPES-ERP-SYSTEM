...
const Payroll: React.FC = () => {
  ...
  // 3. Mutation: Quick Register Staff
  ...
  const runPayroll = useMutation({
    mutationFn: async () => {
      if (!employees || employees.length === 0) throw new Error("Registry is empty.");
      
      const records = employees.map(emp => ({
        employee_id: emp.id,
        pay_month: currentPayMonth,
        gross_amount: emp.gross_salary,
        net_amount: emp.gross_salary * 0.90, // Simplified 10% tax/benefit deduction
        status: 'Paid',
      ...
      // Automatically post to Finance Transactions for consolidated accounting
          ...
          <div className="bg-slate-900 p-6 rounded-3xl shadow-xl border border-slate-800 relative overflow-hidden group">
            <div className="relative z-10">
               ...
               <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest text-slate-500">
                    <span>Est. Tax (TDS)</span>
                    <span className="text-red-400">{formatCurrency(stats.totalSalaries * 0.1)}</span>
                  </div>
                ...
                {payrollHistory?.slice(0, 8).map((record: any) => (
                  <div key={record.id} className="p-3 hover:bg-slate-50 rounded-xl flex items-center justify-between group transition-all border border-transparent hover:border-slate-100">
                    ...
                    <div className="text-right">
                       ...
                       <p className="text-[10px] font-bold text-slate-300">PAID</p>
                    </div>
                  </div>
                ))}
             </div>
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                  <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-200/50">
                    ...
                    <span className="text-2xl font-black text-slate-900">{formatCurrency(stats.totalSalaries * 0.9)}</span>
                  </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200">
              <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                ...
                <button onClick={() => setIsAddStaffOpen(false)} className="text-slate-400 hover:text-slate-600 p-2 hover:bg-slate-50 rounded-full transition-all"><X size={20}/></button>
              </div>
              <form onSubmit={(e) => { e.preventDefault(); addStaff.mutate({...newStaff, gross_salary: parseFloat(newStaff.gross_salary)}); }} className="p-8 space-y-4">
                <div className="grid grid-cols-2 gap-4">
